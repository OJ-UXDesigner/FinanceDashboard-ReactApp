...
import TotalAmountList from "../income-components/TotalAmountList.jsx";
...
import TotalHeading from "../income-components/TotalHeading.jsx";

export default function TotalInvestments(){
    const totalIncomeAmountData = useLoaderData ()
    return(
        ...
        <div className="list-and-report-container filter-and-amount-container w-full h-auto flex flex-col justify-start items-start gap-[1.5rem]">
            <div className="amount-list-container w-full h-auto flex flex-col justify-start items-start gap-8 pt-12">
                {totalIncomeAmountData.map((data, index)=>(
                    <div key={index} className="w-full h-auto flex flex-row justify-between items-center border-l-2 border-[#F76C5E]">
                        <TotalAmountList amountNumber={data.amount} amountDate={data.date}/>
                    </div>
                ))}
            </div>
